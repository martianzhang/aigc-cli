...
| 🔍 | **Prompt Ideas** | Offline BM25 search (CJK-aware + n-gram + RRF) plus 4 keyless online prompt libraries, fused with RRF; 10K+ prompt dataset. |
...
The same `image` / `video` / `audio` / `music` / `models` command automatically uses the correct API path based on the provider:
...
| **OpenLux** | `POST /v1/images/generations` (sync) | `POST /v1/video/create` + `GET /v1/video/query` | ❌ Not yet available | — | `GET /v1/models` |
...
| [Prompt Ideas](docs/en/guide-ideas.md) | Local BM25 search + online libraries (aipromptslibrary, prompts.chat, openart, civitai) |
