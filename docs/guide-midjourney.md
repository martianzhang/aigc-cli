...
命令别名为 `mj`，两种写法等价：
```bash
apimart-cli midjourney imagine --prompt "..."
apimart-cli mj imagine --prompt "..."
```
...
# Fast 模式
apimart-cli mj imagine --prompt "a cute cat" --speed fast
...
```bash
# 基础融合
apimart-cli mj blend --image-url a.png --image-url b.png

# 指定比例
apimart-cli mj blend \
...
apimart-cli mj zoom --task-id task_xxx
```
...
apimart-cli mj pan --task-id task_xxx --direction up
```
...
写入 `~/.config/openai/config.yaml` 或 `~/.config/apimart/config.yaml`：
