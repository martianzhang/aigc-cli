...
`--remove-watermark` 完全重构，默认算法从经典 Alpha Map 逆 alpha 混合升级为 **MI-GAN AI 修补**（基于 Picsart MI-GAN 的 ONNX 推理 inpainting 模型，28MB）：
...
- `init` 自动下载 CPU + GPU 双版 ONNX Runtime（Linux x64 / Windows x64）
- GPU 自动检测：有 CUDA 优先使用 GPU EP，无 GPU 自动降级 CPU
- 新增 `AIGC_CUDA_DEVICE` 环境变量控制 GPU 设备号（`-1` 强制 CPU）
- macOS Apple Silicon 利用内置 CoreML EP（ANE + GPU），无需额外配置
- `aigc-cli detect --remove-watermark --watermark-box "200,60"` — 手动指定水印位置（右下角 200×60）
- `aigc-cli detect --remove-watermark --wmb "800,900,200,60"` — `--wmb` 是 `--watermark-box` 的简写
...
| 算法 | 方式 | 说明 |
|---|---|---|
| **MI-GAN**（默认） | AI 修补 | ONNX 推理，基于 Picsart MI-GAN 的 inpainting 模型。需要 `migan.onnx`（28MB）放在 `~/.config/aigc-cli/models/` |
