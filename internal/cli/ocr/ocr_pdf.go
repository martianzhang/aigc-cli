...
func scanPDF(cmd *cobra.Command, pdfPath string) error {
	...
	op := options.Shared.ResolveProvider(options.ProviderNameOCR)
	if provider.IsOnlineProvider(op) {
		if ocrScanModel != "" {
			op.Model = ocrScanModel
		} else if options.Shared.Model != "" {
			op.Model = options.Shared.Model
		}
		if op.Model != "" {
			useOnlineOCR = true
			onlineP = op
	...
	if useOnlineOCR {
		...
		modelsDir := resolveModelsDir(options.ProviderNameOCR)
