...
import (
	...
	cliocr "github.com/martianzhang/aigc-cli/internal/cli/ocr"
...
func executeGenerateSpeech(argsJSON string) string {
	...
	model := params.Model
	if model == "" {
		if cfg := options.AudioDefaults(); cfg != nil && cfg.SpeakModel != "" {
	...
	filename, err := service.SaveAudioFile(audioData, format, options.Shared.OutputDir)
	if err != nil {
...
func executeTranscribeAudio(argsJSON string) string {
	...
	model := params.Model
	if model == "" {
		if cfg := options.AudioDefaults(); cfg != nil && cfg.TranscribeModel != "" {
...
func executeRecognizeText(argsJSON string) string {
	...
	modelsDir := cliocr.DefaultModelsDir()
	engine, err := cliocr.NewOCREngineWithLang(modelsDir, args.Lang)
	if err != nil {
