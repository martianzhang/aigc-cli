...
func tryInitONNX() *onnx.Detector {
	...
	libPath, err := onnx.DefaultLibPath(filepath.Join(options.ConfigDir(), "models"))
	if err != nil {
		return nil
	}
	...
	if cfg := options.DetectConfig(); cfg != nil && cfg.Model != "" {
...
func detectModelsDir() string {
	...
	return filepath.Join(options.ConfigDir(), "models", "detect")
}
