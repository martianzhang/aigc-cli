...
func generateOnlineBackground(imagePath string, p *provider.EffectiveProvider, defaultPrompt, userPrompt string) (string, error) {
	...
	if p.Type == types.ProviderOllama || provider.IsLocalEndpoint(p.BaseURL) {
		saved, err := service.OllamaGenerateImages(p.BaseURL, req, options.Shared.OutputDir)
		if err != nil {
			return "", err
		}
		if len(saved) == 0 {
	...
	if img.B64JSON != "" {
		return service.SaveBase64Image(options.Shared.OutputDir, prefix, img.B64JSON, 0)
	}
	if img.URL != "" {
		return service.DownloadFile(img.URL, options.Shared.OutputDir, prefix)
	}
