  ...
  aigc-cli kb search "golang context timeout"`,
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		if options.Shared.Cfg == nil {
			if cfg, err := config.Load(options.Shared.CfgFile); err == nil {
				options.Shared.Cfg = cfg
			}
		}
...
func init() {
	kbDir := filepath.Join(options.ConfigDir(), "knowledge")
	vaultDir := filepath.Join(options.ConfigDir(), "vault")
...
func resolveConfigBaseDir() string {
	if cfg := options.KnowledgeDefaults(); cfg != nil {
		return cfg.BaseDir
	}
	return ""
}
...
func shouldAutoSave() bool {
	...
	if cfg := options.KnowledgeDefaults(); cfg != nil {
		return options.Deref(cfg.AutoSave, true)
	}
	return true
}
...
// Cmd returns the knowledge base command tree.
func Cmd() *cobra.Command { return kbCmd }
