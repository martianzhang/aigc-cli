...
func runGeminiImage(c client.APIClient, req *types.GenerateRequest, _ *imageDispatchCtx) ([]string, error) {
	...
	for i, img := range geminiResp.Data {
		if strings.HasPrefix(img.URL, "data:") {
			...
			filename, err := service.SaveBase64Image(options.Shared.OutputDir, prefix, img.URL, i)
			if err != nil {
			...
			if err != nil {
				service.SaveBase64Fallback(options.Shared.OutputDir, fmt.Sprintf("image_%d", time.Now().Unix()), img.URL, 0)
				continue
			}
			...
			if ext == "" {
				ext = service.ExtractImageExt(img.URL)
			}
			if ext == "" {
				ext = ".png"
			}
			filename := filepath.Join(options.Shared.OutputDir, fmt.Sprintf("image_%d_%d%s", time.Now().Unix(), i, ext))
