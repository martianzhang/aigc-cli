...
// ModelInfo 描述一个深度估计模型变体。
type ModelInfo struct {
	// ID 是完整的模型标识（如 depth-anything-v2-small），用于 --model。
	ID string
	...
	// Size 是模型文件大小的人类可读描述。
	Size string
	// License 是模型许可证（显示给用户，CC-BY-NC 禁止商用）。
	License string
}

// Models 是深度估计模型注册表（Depth Anything V2）。
//
// 许可证差异：
...
// DefaultModelID 返回默认模型 ID（Apache-2.0 商用友好，CPU 速度优先）。
...
// ModelPath 返回指定模型的主文件完整路径；未知标识回退到默认模型。
...
// ListModelIDs 返回全部模型 ID（默认在前，其余按字母序），用于 --help 展示。
func ListModelIDs() []string {
	...
	sort.Strings(others)
	return append([]string{DefaultModelID}, others...)
}
